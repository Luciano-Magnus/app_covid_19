...
data class BoletimstatesBrasilHTTP(var uf:String) {
    var teste = uf
    val Json_URL = "https://covid19-brazil-api.now.sh/api/report/v1/brazil/uf/" + "$uf"
    ...
    fun readBoletins(json: JSONArray): List<BoletimStates> {
        val boletins = mutableListOf<BoletimStates>()
        try {
    ...
    @RequiresApi(Build.VERSION_CODES.O)
    fun loadBoletim(): List<BoletimStates>? {
        try {
